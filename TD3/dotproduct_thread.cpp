...
double dot( std::vector<double>& u, std::vector<double>& v )
{
    ...
    std::cout << "Créer " << num_threads << " threads\n";
    ...
    {
        threads[it] = std::thread([&iomutex, it] {
            {
                std::lock_guard<std::mutex> iolock(iomutex);
                std::cout << "Thread #" << it << " is running\n";
            }
            //std::this_thread::sleep_for(std::chrono::millisecons(200));
            int deb, fin;
            if (it < reste)
            {
        ...
        }); // voir si c'est le bon endroit pour cette parenthèse
    }
