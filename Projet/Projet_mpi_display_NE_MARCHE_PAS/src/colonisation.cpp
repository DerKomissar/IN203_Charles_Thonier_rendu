...
int main(int argc, char ** argv)
{
    ...
    int heightLoc = height / nbp; // hauteur des tranches de galaxie
    ...
    while (1) {
        ...
        // partie qu'on va paralléliser avec MPI
        //gr.render(g);
        // voir si gr peut manipuler des trucs plus petits que la taille de la fenêtre ou s'il faut en créer d'autres
        galaxie g_mini(width, heightLoc);
        if (rank == 0) {
            &g_mini = &g;
            gr.render(g_mini);
        } else {
            &g_mini = &g + rank * heightLoc * sizeof(g[0]);
            gr.render(g_mini);
        }
        MPI_Finalize();
