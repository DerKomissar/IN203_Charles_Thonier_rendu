...
int main(int argc, char ** argv)
{
    ...
    long double total_compute_time = 0;
    long double total_display_time = 0;
    double mean_compute_time = 0;
    double mean_display_time = 0;
    ...
    while (1) {
        ...
        std::cout << "Temps passe : "
                  ...
                  << "\r" << std::flush;
        total_compute_time += elaps1.count();
        total_display_time += elaps2.count();
        
        //_sleep(1000);
        if (SDL_PollEvent(&event) && event.type == SDL_QUIT) {
            mean_compute_time = total_compute_time / (temps/deltaT) * 1000;
            mean_display_time = total_display_time / (temps/deltaT) * 1000;
            std::cout << std::endl << "The end" << std::endl;
            std::cout << "Temps de calcul moyen : " << mean_compute_time
