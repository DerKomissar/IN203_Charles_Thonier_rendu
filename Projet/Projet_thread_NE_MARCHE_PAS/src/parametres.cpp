...
//_ ______________________________________________________________________________________________ _
void mise_a_jour(const parametres& params,
                 int width, int height,
                 const char* galaxie_previous, char* galaxie_next)
{
    int i, j;
    
    memcpy(galaxie_next, galaxie_previous, width*height*sizeof(char));
    // il faut indiquer la longueur de ce que l'on copie, d'où le 3e arg
    
    // boucles canoniques -> PRAGMA
    for ( i = 0; i < height; ++i )
    {
        for ( j = 0; j < width; ++j )
        {
            ...
            { // si c'est habité
                ...
                { // et s'il y a des voisins colonisables
                    expansion e = calcul_expansion(params);
                    if (e == expansion_isotrope)
                    { // et si on peut explorer dans toutes les directions
                    ...
                    { // si on n'explore pas dans toutes les directions
                        // Calcul de la direction de l'expansion :
                        int ok = 0;
                        /* Boucle infinie = non-parallélisable
                        Voir du côté des threads voire MPI  */
                        do
                        {
                            int dir = std::rand()%4; // {0,1,2,3}
